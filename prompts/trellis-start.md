...
Initialize a Trellis-managed Pi session. Pi also injects compact startup context through the package extension, but this prompt remains as a manual fallback when you need to reload orientation.
...
```bash
python3 ./.trellis/scripts/get_context.py --mode phase --platform pi
```
